...
var globals = require('./_globals'); //LARAVEL SERVER INFO
...
//recuperate games from unexpected shutdown
client.hvals('games', function(err, hashValues) {
    if(!err){
        let game = {};
        hashValues.forEach(elementStr => {
            gameDB = JSON.parse(elementStr);
            console.log("game " + gameDB.gameID + 
                " was recuperated from an unexpected shutdown!");
            game = new MemoryGame();
            game.gameStartedCallBack = gameStartedCallBack;
            game.callbackAdvanceTurn = turnEndedCallBack;
            game.callbackGameEnded = gameEndedCallBack;
            game.turnPieceCallback = turnPieceCallback;
            game.gameID = gameDB.gameID;
            game.token = gameDB.token;
            game.createdBy = gameDB.createdBy;
            game.createdAt = gameDB.createdAt;
            game.gameEnded = gameDB.gameEnded;
            game.gameStarted = gameDB.gameStarted;
            game.winner = gameDB.winner;
            game.players = gameDB.players;
            game.gameTurn = gameDB.gameTurn;
            game.firstPlay = gameDB.firstPlay;
            game.maxPlayers = gameDB.maxPlayers;
            game.playerTurn = gameDB.playerTurn;
            game.rows = gameDB.rows;
            game.columns = gameDB.columns;
            game.board = gameDB.board;
            game.hidden = gameDB.hidden;
            game.frozen = gameDB.frozen;
            game.botTurn = gameDB.botTurn;
            //add game to ram memory e restart game flow
            games.games.set(game.gameID, game);
            if(game.playerToPlay().bot == false){
                game.callbackAdvanceTurn(game, true);
            }else{
                game.callbackAdvanceTurn(game, false);
                game.playBot();
            }
        });
    } else {
...
io.on('connection', (socket) => {
    ...
    socket.on('get_lobby_chat', () => {
        ...
        for (let i = 0; i < lobbyMessages.length &&
        i < 10; ++i) {
    ...
    socket.on('get_active_game_chat', (data) => {
        ...
        if(!allChatEntries){
            //in case reload from db
            activeGamesMessages.set(data.gameID, []);
            return;
        }
        for (let i = 0; i < allChatEntries.length &&
        i < 10; ++i) {
    ...
    socket.on('create_game', function (data) {
        ...
        if (data.multiPlayer === true) {
            ...
            let game = games.createGame(user.nickname,
                user.token,
                data.gameRows,
                data.gameColumns,
                4,
            ...
            //save game on redis
            client.hset("games", game.gameID, JSON.stringify(game));
    ...
    socket.on('add_bot', function (data) {
        ...
        var randName = namesArray[Math.floor(Math.random() * namesArray.length)];
    ...
    socket.on('leave_active_game', function (data) {
        ...
        if (game !== null) {
            if (game.createdBy == user.nickname) {
                //delete game on redis
                client.hdel("games", game.gameID);
    ...
    socket.on('game_click', function (data) {
        ...
        if (!data || typeof data.row === 'undefined' || isNaN(data.row)) {
            return;
        }
        if (!data || typeof data.column === 'undefined' || isNaN(data.column)) {
            return;
        }
        //IF CAN PLAY ON GAME
...
let turnEndedCallBack = (g, notify) => {
    //update game on redis
    client.hset("games", g.gameID, JSON.stringify(g));
